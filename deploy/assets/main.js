...
// Einwilligung wird gemerkt (localStorage) -> bei Folgebesuchen laedt die Karte automatisch.
const mapLoad = document.getElementById('mapLoad');
function loadKrebberMap(box) {
  if (!box || typeof L === 'undefined' || box.classList.contains('map-loaded')) return;
...
if (mapLoad) {
  ...
  // Bereits eingewilligt? -> automatisch laden (kein erneuter Klick noetig)
  let consented = false;
  try { consented = localStorage.getItem('mapConsent') === '1'; } catch (e) {}
  if (consented) loadKrebberMap(box);
}

// Kontaktformular -> Direkt-Versand an contact.php (eigener Server, kein Fremd-Dienst)
const contactForm = document.getElementById('contactForm');
if (contactForm) {
  const cfStatus = document.getElementById('cfStatus');
  const showStatus = (msg, kind) => {
    if (!cfStatus) return;
    cfStatus.className = 'cf-status' + (kind ? ' ' + kind : '');
    cfStatus.innerHTML = msg;
  ...
  contactForm.addEventListener('submit', async e => {
    ...
    if (cfStatus) cfStatus.hidden = true;
    try {
      ...
      if (res.ok && data.ok) {
        contactForm.reset();
        showStatus('Danke! Deine Nachricht wurde gesendet – ich melde mich zeitnah bei dir zurück.', 'ok');
      } else {
        showStatus((data && data.error) ? data.error : 'Das hat leider nicht geklappt.', 'err');
      }
    } catch (err) {
      showStatus('Verbindung fehlgeschlagen. Schreib mir gern direkt an <a href="mailto:mail@physio-martinkrebber.de">mail@physio-martinkrebber.de</a> oder ruf an.', 'err');
    } finally {
